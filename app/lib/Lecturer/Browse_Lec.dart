...
import 'package:app/Lecturer/Test_To_Borrow.dart';
...
class BrowseLec extends StatelessWidget {
  const BrowseLec({super.key});
  ...
  Future<void> fetchBorrowedData() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('http://192.168.1.189:3000/total/approved'),
        headers: {
  ...
  void _onItemTapped(int index) {
    ...
    switch (_selectedIndex) {
      ...
      case 3:
        ...
        break;
      default:
        nextPage = const BrowseLec(); // Default to Home page
    }
                ...
                onTap: isAvailable
                    ? () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => Borrow_Lec(bookId: bookId),
                          ),
                        );
                      }
                    : null, // Make the card unclickable if the book is not available
                child: Opacity(
                  opacity:
                      isAvailable ? 1.0 : 0.5, // Dim the card if not available
                  child: Column(
                    children: [
                      Container(
                        width: imageWidth,
